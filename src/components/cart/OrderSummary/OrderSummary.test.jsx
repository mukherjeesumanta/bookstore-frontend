...
const items = [
  { id: '1', title: 'Clean Code', author: 'Robert C. Martin', price: 34.99, quantity: 1, cover: '' },
  { id: '2', title: 'Design Patterns', author: 'Gang of Four', price: 44.99, quantity: 2, cover: '' },
];

describe('OrderSummary', () => {
  it('renders the Order Summary heading', () => {
    render(<OrderSummary items={[]} subtotal={0} shippingCost={0} tax={0} total={0} />);
    expect(screen.getByText('Order Summary')).toBeInTheDocument();
  });

  it('renders each item title', () => {
    render(<OrderSummary items={items} subtotal={124.97} shippingCost={5.99} tax={9.00} total={139.96} />);
  ...
  it('renders subtotal, shipping and tax rows', () => {
    render(<OrderSummary items={[]} subtotal={20} shippingCost={5.99} tax={1.60} total={27.59} />);
  ...
  it('renders correct subtotal value', () => {
    render(<OrderSummary items={[]} subtotal={20.00} shippingCost={5.99} tax={1.60} total={27.59} />);
  ...
  it('shows overflow count when items exceed maxItems', () => {
    const manyItems = Array.from({ length: 7 }, (_, i) => ({
      id: String(i), title: `Book ${i}`, author: 'Author', price: 10, quantity: 1, cover: '',
    }));
    render(<OrderSummary items={manyItems} maxItems={5} subtotal={70} shippingCost={0} tax={0} total={70} />);
    expect(screen.getByText('+ 2 more item(s)')).toBeInTheDocument();
  });

  it('renders optional footer slot', () => {
    render(
      <OrderSummary items={[]} subtotal={0} shippingCost={0} tax={0} total={0}
