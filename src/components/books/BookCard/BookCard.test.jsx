...
describe('BookCard', () => {
  ...
  it('renders an Add to Cart button when in stock', () => {
    renderCard();
    expect(screen.getByRole('button', { name: /add the pragmatic programmer to cart/i })).toBeInTheDocument();
  });
  ...
  it('calls addItem on Add to Cart click', async () => {
    renderCard();
    const btn = screen.getByRole('button', { name: /add the pragmatic programmer to cart/i });
