  ...
  return (
    <article className="bg-white rounded-xl border border-[#E8E4D9] p-4 flex flex-col hover:shadow-md transition-shadow">
      <div className="flex-1 mb-3">
        <h3 className="font-semibold text-[#1C1C1C] text-sm mb-0.5">{book.title}</h3>
        <p className="text-xs text-[#7A7A68] mb-2">{book.author}</p>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          <StarRating rating={book.rating} />
          <a href="#" className="text-xs text-[#C4622D] hover:underline">Review</a>
        </div>
      </div>
      <div className="flex items-end justify-between gap-3">
        <div>
          <Link to={`/books/${book.id}`} className="block overflow-hidden rounded-md w-14 aspect-[2/3] bg-[#F0EDE4]">
            <img
              src={book.cover}
              alt={`Cover of ${book.title}`}
        ...
        <div className="flex-1 text-right">
          <p className="font-bold text-[#1C1C1C] text-sm mb-2">${book.price.toFixed(2)}</p>
          <button
            onClick={() => addItem(book)}
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{ background: "linear-gradient(135deg, #2C3A1E 0%, #4A5C34 100%)" }}
      >
        <div
          className="absolute inset-0 bg-cover bg-center opacity-40"
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=1400&q=80')" }}
          aria-hidden="true"
        />
        <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-16">
          <div className="max-w-lg">
            ...
            <p className="text-[#D4CFC0] text-base mb-8 leading-relaxed">
              Explore our curated collection of diverse voices and timeless classics.
            </p>
            <Link
              to="/catalogue"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <h2 className="text-2xl font-bold text-[#1C1C1C] mb-6">Featured This Month</h2>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <button className="w-8 h-8 rounded-full border border-[#D4CFC0] flex items-center justify-center text-[#5C5C4F] hover:border-[#2C3A1E] hover:text-[#2C3A1E] transition-colors" aria-label="Previous">
              ‹
            </button>
            <button className="w-8 h-8 rounded-full border border-[#D4CFC0] flex items-center justify-center text-[#5C5C4F] hover:border-[#2C3A1E] hover:text-[#2C3A1E] transition-colors" aria-label="Next">
              ›
            </button>
          </div>
        </div>
      ...
      <section className="bg-[#2C3A1E] py-14">
        <div className="max-w-xl mx-auto px-4 text-center">
          <h2 className="text-2xl font-bold text-[#FAF9F5] mb-2">Stay in the Know</h2>
          <p className="text-[#B0B898] text-sm mb-6">
            Subscribe to our newsletter for new releases, recommendations, and events.
          </p>
          <form
