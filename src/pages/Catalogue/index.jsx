  ...
  return (
    <div className="border-b border-[#E8E4D9] pb-4 mb-4">
      ...
      >
        {title}
        <ChevronDown
          className={`w-4 h-4 text-[#7A7A68] transition-transform ${open ? "" : "-rotate-90"}`}
        />
      </button>
...
export default function Catalogue() {
    ...
    if (selectedCat !== "All") {
      result = result.filter(
        (b) =>
          ...
          b.tags?.some((t) =>
            t.toLowerCase().includes(selectedCat.toLowerCase()),
          ),
      );
    }
    if (search.trim()) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-[#1C1C1C] mb-6">
      ...
      <div className="flex gap-8">
        ...
        <aside className="hidden lg:block w-56 flex-shrink-0">
          <p className="text-sm font-semibold text-[#7A7A68] mb-4">
            Optional Filter
          </p>
          ...
          <FilterSection title="Titles">
            <ul className="space-y-1.5 text-sm text-[#5C5C4F]">
              <li>
                <button className="hover:text-[#C4622D]">Titles</button>
              </li>
              <li>
                <button className="hover:text-[#C4622D]">Genres</button>
              </li>
            </ul>
          </FilterSection>
          ...
          <FilterSection title="Price Range" defaultOpen={false}>
            <div className="flex gap-2 items-center text-sm text-[#5C5C4F]">
              <input
                type="number"
                placeholder="Min"
                className="w-20 px-2 py-1 rounded border border-[#E8E4D9] text-xs focus:outline-none"
              />
              <span>–</span>
              <input
                type="number"
          ...
          <FilterSection title="Authors">
            <ul className="space-y-1.5 text-sm text-[#5C5C4F]">
              {[
                ...
                <li key={a}>
                  <button className="hover:text-[#C4622D]">{a}</button>
                </li>
              ))}
            </ul>
          </FilterSection>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-center gap-1 mt-10">
            <button className="w-8 h-8 flex items-center justify-center rounded-full text-[#5C5C4F] hover:bg-[#E8E4D9] transition-colors">
              ‹
            </button>
            {[1, 2, 3, 4].map((p) => (
              <button
