...
describe('Payment page', () => {
  ...
  it('renders PayPal option', () => {
    ...
    expect(screen.getByText((_, el) =>
      el?.tagName === 'LABEL' && el.textContent.toLowerCase().includes('paypal')
    )).toBeInTheDocument();
  });
  ...
  it('renders lock security notice', () => {
    renderPayment();
    expect(screen.getByText(/all transactions are secured/i)).toBeInTheDocument();
  });

  it('shows PayPal button when PayPal radio is selected', async () => {
    renderPayment();
    // Find label whose text content includes "paypal"
    const paypalLabel = screen.getByText((_, el) =>
      el?.tagName === 'LABEL' && el.textContent.toLowerCase().includes('paypal')
    );
